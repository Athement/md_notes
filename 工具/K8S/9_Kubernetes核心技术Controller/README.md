...
Pod 和 Controller之间是通过**label标签**来建立关系，同时Controller又被称为控制器（工作负载）
...
上述代码不能复用，因为每次我们都需要重新输入代码，所以我们都是通过YAML进行配置
...
以上方式创建的控制器只能在集群内部进行访问，所以我们还需要对外暴露端口
...
我们在下载 1.15版本，容器就处于ContainerCreating状态，然后下载完成后，之后逐个pod用 1.15版本去替换1.14版本了，这么做的好处就是：升级可以保证服务不中断
