...
![git操作通用流程](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/25/162fcc0987bf1c0a~tplv-t2oaga2asx-watermark.awebp)
...
![git几个核心区域间的关系](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/4/25/162fcc0e7e711dc7~tplv-t2oaga2asx-watermark.awebp)
...
5. 将本地仓库某分支推送到远程仓库上
   ...
   - 远程已有分支并且已经关联本地分支，并且本地已经切换到本地分支

     `git push [remote-name] [branch-name]`，如`git push origin master`

   - 远程已有分支但未关联本地分支，并且本地已经切换到本地分支

     `git push -u [remote-name] [branch-name]`,如`git push -u origin/master`

   - 如果想将本地分支推送到远程仓库的不同名分支

     `git push <remote-name> <local-branch>:<remote-branch>`，如`git push origin serverfix:awesomebranch`;

   - 如果想删除远程分支

     `git push [romote-name] :<remote-branch>`，如`git push origin :serverfix`。相当于将空白内容推送给远程分支

6. 查看远程仓库的详细信息：`git remote show origin`；

7. 修改某个远程仓库在本地的简称：`git remote rename [old-name] [new-name]`，如`git remote rename origin org`；

8. 移除远程仓库：`git remote rm [remote-name]`；
